...
public class PhenopacketValidationTests {
    ...
    void testPhenopacketValidity() throws IOException, URISyntaxException {
        ...
        CrudRepository<Phenopacket, Phenopacket.Key> repo =
                repositoryProvider.getCrudRepository(ResolvableType.forClass(Phenopacket.class));
        ...
        ValidationWorkflowRunner<org.phenopackets.schema.v2.PhenopacketOrBuilder> runner =
                JsonSchemaValidationWorkflowRunner.phenopacketBuilder().build();
    ...
    @Test
    @Transactional
    void testPhenopacketValidityRead() throws IOException, URISyntaxException {
        // 1) Load your transaction JSON
        String json = Files.readString(
                Paths.get(Objects.requireNonNull(
                                getClass().getClassLoader()
        ...
        HttpHeaders phenoHeaders = new HttpHeaders();
        phenoHeaders.setAccept(List.of(MediaType.valueOf("application/json")));

        var phenoResp = restTemplate.exchange(URI.create("http://localhost:" + port + "/pheno/Phenopacket/phenofhir" +
                ":composition"), HttpMethod.GET, new HttpEntity<>(phenoHeaders), String.class);

        assertThat(phenoResp.getStatusCode()).isEqualTo(HttpStatus.OK);
        log.info("Loaded: {}", phenoResp.getBody());

        ValidationWorkflowRunner<org.phenopackets.schema.v2.PhenopacketOrBuilder> runner =
                JsonSchemaValidationWorkflowRunner.phenopacketBuilder().build();
        var results = runner.validate(phenoResp.getBody());
        for (var r : results.validationResults()) {
            log.info("Validator   {}", r.validatorInfo().validatorName());
            log.info("Description {}", r.validatorInfo().description());
            log.info("{}", r.message());
